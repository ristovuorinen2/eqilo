...
## Implementation Plan
1. **Phase 1: Setup, Data Modeling & Auth** - Initialize Next.js App Router project, Firebase environment, and Tailwind CSS (Shadcn UI + Radix). Define Firestore collections (`customers`, `products`, `categories`, `orders`, `carts`, `settings`). Setup flexible Firebase Auth (Email/Phone) and secure Server Actions architecture.
2. **Phase 2: Admin Core (Catalog & CRM)** - Build the one-time Excel import logic from `Price List 2026 V3.0.xlsx` and the `fdstiming.com` scraping/translation script. Develop the Admin Panel featuring a Product Editor, CRM for managing `customers`, Cart management (shareable links/price overrides), and a Sales Dashboard.

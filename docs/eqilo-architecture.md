...
## Implementation Plan
1. ✅ **Phase 1: Setup, Data Modeling & Auth (COMPLETE)** - Initialize Next.js App Router project, Firebase environment, and Tailwind CSS (Shadcn UI + Radix). Define Firestore collections (`customers`, `products`, `categories`, `orders`, `carts`, `settings`). Setup anonymous Firebase Auth and secure Server Actions architecture.
2. ✅ **Phase 2: Admin Core (Catalog & CRM) (COMPLETE)** - Build the one-time Excel import logic and the `fdstiming.com` scraping/translation script. Develop the Admin Panel featuring a Product Editor, CRM, Cart management, and detailed VAT breakdown tracking.
